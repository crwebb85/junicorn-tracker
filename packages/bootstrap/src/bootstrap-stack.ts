...
export interface BootstrapStackProps extends StackProps {
  ...
  repository: string;
  branch: string; //can be a glob string
}
...
export class BootstrapStack extends Stack {
  constructor(scope: Construct, id: string, props: BootstrapStackProps) {
     ...
     */
    const provider = new OpenIdConnectProvider(this, 'OpenIdProvider', {
    ...
    const GitHubPrincipal = new OpenIdConnectPrincipal(provider).withConditions({
      StringLike: {
        'token.actions.githubusercontent.com:sub': `repo:${props.githubOrganisation}/${props.repository}:${props.branch}`,
      },
    });
